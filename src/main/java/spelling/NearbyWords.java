...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
        ...
        // COMPLETED: Implement the remainder of this method, see assignment for algorithm
		// insert first node
		queue.add(word);
		while (!queue.isEmpty() && retList.size() < numSuggestions && visited.size() < THRESHOLD) {
		    String wordToVisit = queue.remove(0);
            visited.add(wordToVisit);
		    List<String> wordsDistanceOne = distanceOne(wordToVisit, false);
		    for (String wordDistanceOne : wordsDistanceOne) {
                if (visited.contains(wordDistanceOne)) {
                    continue;
                }

                visited.add(wordDistanceOne);
                queue.add(wordDistanceOne);

                if (dict.isWord(wordDistanceOne)) {
                    retList.add(wordDistanceOne);
